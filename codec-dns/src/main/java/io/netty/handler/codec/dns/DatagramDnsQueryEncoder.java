...
public class DatagramDnsQueryEncoder extends MessageToMessageEncoder<AddressedEnvelope<DnsQuery, InetSocketAddress>> {
    ...
    protected void encode(
        ...
        try {
            ...
            encodeAdditionalRecords(query, buf);
            success = true;
        } finally {
            if (!success) {
                buf.release();
    ...
    private static void encodeHeader(DnsQuery query, ByteBuf buf) {
        ...
        buf.writeShort(query.questionCount());
        buf.writeShort(0); // answerCount
        buf.writeShort(0); // authorityResourceCount
        buf.writeShort(query.additionalRecordCount());
    }

    private void encodeQuestions(DnsQuery query, ByteBuf buf) throws Exception {
        final int questionCount = query.questionCount();
        if (questionCount == 1) {
            recordEncoder.encodeRecord(query.question(), buf);
        } else if (questionCount != 0) {
            for (DnsQuestion q: query.questions()) {
                recordEncoder.encodeRecord(q, buf);
            }
        }
    }

    private void encodeAdditionalRecords(DnsQuery query, ByteBuf buf) throws Exception {
        final int additionalRecordCount = query.additionalRecordCount();
        if (additionalRecordCount == 1) {
            recordEncoder.encodeRecord(query.additionalRecord(), buf);
        } else if (additionalRecordCount != 0) {
            for (DnsRecord r: query.additionalRecords()) {
                recordEncoder.encodeRecord(r, buf);
            }
        }
    }
}
