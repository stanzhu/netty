...
import io.netty.handler.codec.dns.DnsRecordClass;
...
public class DnsNameResolver extends SimpleNameResolver<InetSocketAddress> {
    ...
    private volatile DnsRecordClass maxPayloadSizeClass; // EDNS uses the CLASS field as the payload size field.
    ...
    public DnsNameResolver setMaxPayloadSize(int maxPayloadSize) {
        ...
        maxPayloadSizeClass = DnsRecordClass.valueOf(maxPayloadSize);
        ch.config().setRecvByteBufAllocator(new FixedRecvByteBufAllocator(maxPayloadSize));

        return this;
    }

    DnsRecordClass maxPayloadSizeClass() {
    ...
    private final class DnsResponseHandler extends ChannelInboundHandlerAdapter {
            ...
            try {
                ...
                if (res.code() == DnsResponseCode.NOERROR) {
                    ...
                    qCtx.retry(res.sender(),
                               ...
                               " with " + res.answerCount() + " answer(s) and " +
                               res.authorityRecordCount() + " authority resource(s)");
                }
            } finally {
        ...
        private void cache(DnsQuestion question, AddressedEnvelope<? extends DnsResponse, InetSocketAddress> res) {
            ...
            if (resc.answerCount() != 0) {
                for (DnsRecord r : resc.answers()) {
